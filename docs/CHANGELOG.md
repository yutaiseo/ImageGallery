...
- `ctrol/login.php`：从空壳重定向实现为完整登录处理器
  - SHA256 密码验证（使用 `password_verify()` 和 `password_hash(PASSWORD_DEFAULT)`）
...
1. 添加后台轻量框架（`gadmin/bootstrap.php` / `auth.php` / `csrf.php` / `header.php` / `footer.php`）
2. 完善登录流程（`login.php`：设置 `$_SESSION['username']`，调用 `admin_on_login()` 并跳转到 `gadmin/index.php`）
3. 新增管理页面：
   - `gadmin/index.php`：简易仪表盘（用户数、图片数）
   - `gadmin/users.php`：用户管理（分页、搜索、单/批量删除、日志）
   - `gadmin/images.php`：图片管理（分页、搜索、软删除、批量操作、下载按钮）
   - `gadmin/recycle.php`：回收站（恢复、永久删除、批量操作、下载按钮）
   - `gadmin/logs.php`：管理员操作日志查看与 CSV 导出
...
6. 操作日志：新增 `gadmin/logger.php`，将管理员关键动作写入 `user_action_logs` 表，便于审计。

## 验收与测试步骤

1. 登录：访问 `login.php`，使用管理员账号登录，应跳转到 `gadmin/index.php` 并显示统计。 
2. 用户管理：进入 `gadmin/users.php`，尝试搜索、分页，单个删除与多选批量删除，确认 `user_action_logs` 有对应记录。 
3. 图片管理：进入 `gadmin/images.php`，搜索分页，尝试单个删除（应移入回收站）、多选批量删除（应移入回收站），并查看 `gadmin/recycle.php`。 
4. 回收站：在 `gadmin/recycle.php` 恢复或永久删除图片，确认永久删除会从磁盘移除文件，并记录日志。 
5. 日志导出：在 `gadmin/logs.php` 点击导出 CSV，下载文件能打开且包含当前页日志。 
